...
if ($clients_result !== false) {
    $row = sqlsrv_fetch_array($clients_result, SQLSRV_FETCH_ASSOC);
    $total_clients = $row['Count'] ?? 0;
}

$messages_result = sqlsrv_query($conn, "SELECT COUNT(*) as Count FROM Messages");
if ($messages_result !== false) {
    $row = sqlsrv_fetch_array($messages_result, SQLSRV_FETCH_ASSOC);
    $messages_count = $row['Count'] ?? 0;
}
?>
<!DOCTYPE html>
<html lang="en">
